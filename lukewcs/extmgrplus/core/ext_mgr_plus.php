...
class ext_mgr_plus
{
	...
	public function ext_manager($event)
	{
		...
		add_form_key('lukewcs_extmgrplus_settings');

		if ($this->request->is_set_post('submit') && $this->request->variable('extmgrplus_save_settings', '') == '1')
		{
			if (!check_form_key('lukewcs_extmgrplus_settings'))
			{
	...
	{
		$this->config->set('extmgrplus_exec_todo', 0);

		if ($this->config['extmgrplus_todo_purge_cache'])
		{
			$this->config->set('extmgrplus_todo_purge_cache', 0);
			$this->cache->purge();
		}

		if ($this->config_text->get('extmgrplus_todo_add_log') != '')
		{
			$last_job = json_decode($this->config_text->get('extmgrplus_todo_add_log'), true);
			$this->config_text->set('extmgrplus_todo_add_log', '');
...
// var_dump($ext_marked);
	...
	private function enable_disable($action)
	{
		...
		if ($action == "disable")
		{
			...
			$ext_count_success = 0;
// var_dump($ext_list_enabled);

			$this->config->set('extmgrplus_exec_todo', 1);
			$this->config->set('extmgrplus_todo_purge_cache', 1);
			...
			{
				$this->config_text->set('extmgrplus_todo_add_log', $this->get_log_json(
...
// var_dump($ext_list_disabled);
			...
			$this->template->assign_vars([
				'EXTMGRPLUS_LAST_EXT_NAME'			=> '',
				'EXTMGRPLUS_LAST_EXT_DISPLAY_NAME'	=> '',
			]);
